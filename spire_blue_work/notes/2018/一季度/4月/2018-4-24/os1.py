...
try:  
    ...
    if variate1.isdigit() and variate2.isdigit() and  variate3.isdigit() and int(variate1)>1 and int(variate3)>1:  
        print "输对了呢,输入是数字 \n".decode('utf-8')  
        for item in str(sys.argv[2]):  
            if int(item)>=int(str(sys.argv[1])):  
                print "别闹,".decode('utf-8')+str(sys.argv[1])+" 进制怎么可能有 ".decode('utf-8')+str(item)+" ?"  
                sys.exit()  
        if N_2_decimal(int(variate2),int(variate1))>999999999:  
            print "数字太大 电脑君不想工作了......".decode('utf-8')  
            sys.exit()  
    else:  
        print "别闹,输入的不太对啊 ".decode('utf-8')  
        sys.exit()  
    ...
    try:  
        ...
        print "运算出错了呢 先溜为上.....".decode('utf-8')  
        sys.exit()  
    print str(variate1)+" 进制的 ".decode('utf-8')+str(variate2)+" 转成 ".decode('utf-8')+str(variate3)+" 进制的数为 ".decode('utf-8')+rev_value  
except:  
    print "貌似已怀孕......  ".decode('utf-8')  
    print "轻轻的，我走了  ".decode('utf-8')  
    sys.exit()  