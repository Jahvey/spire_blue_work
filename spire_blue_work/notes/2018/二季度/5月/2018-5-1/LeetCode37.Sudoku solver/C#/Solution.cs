public class Solution {
    ...
    private char[,] board;  
        ...
        public void solveSudoku(char[,] board) {  
            this.board=board;  
            this.rows=new bool[9,9];//rows[i,j]第i行是否已经使用了值j+1  
            this.cols=new bool[9,9];//cols[i,j]第i列是否已经使用了值j+1  
            this.subBox=new bool[9,9];//subBox[i,j]第i个小九宫格是否已经使用了值j+1。九宫格按行来，每行从左往右,board[i,j]说属的小九宫格为(i/3)*3+j/3;(i/3)*3为基数(利用除,去尾数),j/3为相对偏移  
            //1.初始化标记  
            int val;  
            for(int i=0;i<9;i++){  
                for(int j=0;j<9;j++){  
        ...
        private bool fillSudoku(int i,int j){  
            if(i==9){//已经成功填完  
                return true;  
            }  
            //计算下一个位置  
